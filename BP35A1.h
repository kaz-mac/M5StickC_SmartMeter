...
public:
    ...
    const uint32_t MEASURE_LIMIT = 1;   //15;
...
public:
    ...
    void commandBin(byte *cmd, size_t len) {
        Serial.println(">> "+String(reinterpret_cast<char*>(cmd)));
...
public:
    ...
    bool noEcho(void) {
        ...
        if (! is_received_res){
            Serial.println("noEcho() nores err");
            return false;
        }
        Serial.println("noEcho() ok");
        return true;
    }
    ...
    bool setWOPT(void) {
        ...
        if (is_received_res) {
            if (res_str.startsWith("OK 00")) {
                Serial.println("setWOPT()) ROPT Binary mode now");
                binmode = true;
            } if (res_str.startsWith("OK 01")) {
                Serial.println("setWOPT()) ROPT Ascii mode now");
            } else {
                Serial.println("unknown result: "+res_str);
            }
        } else {
            Serial.println("setWOPT()) ROPT nores err");
            return false;
        }
        // バイナリモードだったら変更する
        if (binmode) { 
            ...
            bool is_received_res = waitExpectedRes(WAIT_TIME, "OK");
            if(!is_received_res){
                Serial.println("setWOPT()) WOPT nores err");
                return false;
            }
            Serial.println("setWOPT() WOPT OK");
        }
        return true;
    };
...
public:
    ...
    bool clearSession(void) {
        ...
        if (res_str.startsWith("OK")) {
            Serial.println("clearSession()) old session cleared!");
        } if (res_str.startsWith("FAIL ER10")) {
            Serial.println("clearSession()) no old session");
        } else {
            Serial.println("clearSession() other result: "+res_str);
            return false;
        }
        return true;
    };
